...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  5 15:04:45 2019
...
class dual_channel_batchnorm2d(nn.Module):
    def __init__(self, size, eps, alpha=0.5):
        ...
        self.batcher_h = nn.BatchNorm2d(int(size*alpha), eps)
        self.batcher_l = nn.BatchNorm2d(size - int(size*alpha), eps)
    def forward(self, x):
        x_h, x_l = x
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.conv_blocks = nn.Sequential(
            ...
            OctaveConv(128, 128, 3, stride=1, padding=1, alpha_in = 0, alpha_out=out_w),
            dual_channel_batchnorm2d(128, 0.8,alpha=out_w),
            dual_channel_leakyrelu(0.2, inplace=True,alpha=out_w),
            dual_channel_upsample(scale_factor=2,alpha=out_w),
            OctaveConv(128, 64, 3, stride=1, padding=1,alpha_in=out_w, alpha_out=in_w),
            dual_channel_batchnorm2d(64, 0.8,alpha=in_w),
            dual_channel_leakyrelu(0.2, inplace=True,alpha=in_w),
